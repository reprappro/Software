...
class PronterWindow(wx.Frame,pronsole.pronsole):
    def __init__(self, filename=None,size=winsize):
        ...
        self.settings.build_dimensions = '140x140x100+0+0+0' #default build dimensions are 200x200x100 with 0,0,0 in the corner of the bed
        self.settings.relative_extruder = True #Are we working with an extruder that are using relative or absolute positioning
    ...
    def loadviz(self):
        if (self.settings.relative_extruder):
            Xtot,Ytot,Ztot,Xmin,Xmax,Ymin,Ymax,Zmin,Zmax = pronsole.measurements_relative_extruder(self.f)
            print pronsole.total_length_relative_extruder(self.f), _("mm of filament used in this print\n")
        else:
